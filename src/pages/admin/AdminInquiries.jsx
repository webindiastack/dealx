...
export const AdminInquiries = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stage Tab Filters */}
      <div className="bg-white p-2 rounded-2xl border border-slate-200/90 shadow-sm flex flex-wrap gap-1">
        {STAGES.map((stage) => {
          const count =
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-sm flex items-center justify-between">
        ...
        <span className="text-xs font-mono text-slate-500 font-bold hidden sm:inline">
          Inquiries: <strong className="text-slate-900">{filteredInquiries.length}</strong>
        </span>
      </div>
              ...
              {filteredInquiries.map((inq) => (
                <tr key={inq.id} className="hover:bg-slate-50 transition-colors">
                  <td className="py-3.5 px-4">
                    ...
                    <div className="text-[11px] text-slate-500 font-medium">{inq.customerEmail}</div>
                  ...
                  <td className="py-3.5 px-4 max-w-xs">
                    ...
                    <div className="text-[11px] font-mono text-brand-600 font-bold">
                      {formatCurrency(inq.productPrice)}
                    </div>
                  </td>
                  <td className="py-3.5 px-4 max-w-xs">
                    <p className="text-slate-600 line-clamp-2 text-[11px] italic font-medium">
                      "{inq.message}"
                    </p>
                    {inq.adminNotes && (
                      <span className="inline-block mt-1 text-[10px] text-slate-800 font-bold truncate max-w-xs">
                        Notes: {inq.adminNotes}
                      </span>
                    )}
                  </td>
                  <td className="py-3.5 px-4 text-slate-500 text-[11px] font-mono font-medium">
                  ...
                  <td className="py-3.5 px-4 text-right space-x-2">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-slate-100 border border-slate-200 text-slate-700 hover:text-slate-900 hover:border-brand-500 text-xs font-bold transition-colors"
                    >
                      Manage & Notes
                    </button>
                    <button
                      onClick={() => deleteInquiry(inq.id)}
                      className="p-1.5 rounded-lg bg-slate-100 border border-slate-200 text-slate-500 hover:text-rose-600 hover:border-rose-500 transition-colors"
          ...
          <div className="w-full max-w-2xl bg-white rounded-3xl border border-slate-200 shadow-2xl p-6 space-y-5 max-h-[90vh] overflow-y-auto custom-scrollbar">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">Inquiry ID: {selectedInquiry.id}</p>
              </div>
              <button
            ...
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-2">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
                <div>
                  <span className="text-slate-500 block text-[10px] font-bold">Name</span>
                  <strong className="text-slate-900">{selectedInquiry.customerName}</strong>
                </div>
                <div>
                ...
                <div>
                  <span className="text-slate-500 block text-[10px] font-bold">Phone</span>
                  <strong className="text-slate-900">{selectedInquiry.customerPhone}</strong>
                </div>
                <div>
                  <span className="text-slate-500 block text-[10px] font-bold">Location</span>
                  <strong className="text-slate-900">
                    {selectedInquiry.customerCity}, {selectedInquiry.customerState}
                  </strong>
                </div>
                <div>
                  <span className="text-slate-500 block text-[10px] font-bold">Customer Budget</span>
                  <strong className="text-brand-600">{selectedInquiry.customerBudget}</strong>
                </div>
                <div>
                  <span className="text-slate-500 block text-[10px] font-bold">Target Date</span>
            ...
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0 text-xs">
                <span className="text-[10px] uppercase font-mono text-brand-600 font-bold">
                  {selectedInquiry.productCategory}
                </span>
                <h4 className="font-extrabold text-slate-900 truncate">{selectedInquiry.productTitle}</h4>
                <p className="font-mono font-extrabold text-brand-600">
            ...
            <div className="space-y-1">
              <label className="block text-xs font-bold text-slate-700">Customer Note</label>
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-700 italic font-medium">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:outline-none focus:border-brand-500 font-semibold"
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:outline-none focus:border-brand-500"
                />
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-slate-100 flex justify-end gap-3">
              ...
              <button
                ...
                className="px-6 py-2.5 rounded-xl bg-brand-600 text-white text-xs font-bold hover:bg-slate-900 shadow-md"
              >
                Save CRM Updates
              </button>
            </div>
